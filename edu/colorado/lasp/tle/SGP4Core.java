...
//	           }
  ...
  public SGP4Core(TwoLineElement TLE) {
	...
	*    vallado, crawford, hujsak, kelso  2006
    ...
    if ((em >= 1.0) || (em < -0.001))throw new SGP4Exception(String.format("Mean eccentricity out of range %f",em),1,tsince);
    if ((am < 0.95)) throw new SGP4Exception(String.format("Mean semimajor axis out of range",am),1,tsince);
    if (em < 0.0) em  = 1.0e-6;
    mm     = mm + this.no * templ;
    xlm    = mm + argpm + nodem;
    emsq   = em * em;
    ...
    if (this.isdeep) {
      ...
      if ((ep < 0.0 ) || ( ep > 1.0)) throw new SGP4Exception("Perturbed eccentricity out of range",3,tsince);
  ...
  public static void main(String args[]) throws IOException {
    ...
    // ---------------------------- locals -------------------------------
    double tsince, startmfe, stopmfe, deltamin;
      ...
      if (S != null) {
        ...
        while ((tsince < stopmfe) && (error == 0)) {
          ...
          try {
            ...
            System.out.printf("# *** error: t:= %f *** code = %3d - %s\n", ex.tsince,
                ex.code,ex.toString());
            error=ex.code;
          }
