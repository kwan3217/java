...
/**
 * A Reader which implements C <code>scanf</code> functionality. It allows an
 * application to read various primitive types from the underlying stream using
 * <code>scan</code> methods that implement <code>scanf</code> type input
 * formatting.
 * 
 * <p>
 * There are <code>scan</code> methods to read float, double, long, int, char,
 * char[], and Strings, and each can accept a format string with the same syntax
 * as that accepted by the C <code>scanf()</code> function (the exact syntax
 * is described in the documentation for {@link cformat.ScanfFormat ScanfFormat}).
 * 
 * <p>
 * Because Java does not permit variable-length argument lists, only one
 * conversion can be performed per method call. As an example, suppose we wish
 * to parse the input
 * 
 * <pre>
 ...
 * 
 * <p>
 * The scan methods can also be called without an argument, in which case a
 * default format string is used. One can also pre-parse the format string into
 * an object of type {@link cformat.ScanfFormat ScanfFormat}, which is more
 * efficient when a lot of input is being read:
 * 
 * <pre>
 ...
 * 
 * <p>
 * If the input does not match the specified format, then a
 * {@link org.kwansystems.tools.ScanfMatchException ScanfMatchException} is thrown. A
 * <code>java.io.EOFException</code> is thrown if the end of input is reached
 * before the scan can complete successfully, and errors in the underlying input
 * will throw a <code>java.io.IOException</code>. Finally, an invalid format
 * string (or {@link org.kwansystems.tools.ScanfFormat ScanfFormat} object) will trigger an
 * <code>InvalidArgumentException</code>. In the case of a
 * <code>ScanfMatchException</code>, scanning stops at the first character
 * from which it can be determined that the match will fail. This character is
 * remembered by the stream (see the discussion of the look-ahead character,
 * below) and will be the first character seen by the next <code>scan</code>
 * or <code>read</code> method which is called.
 * 
 * <p>
 * The class keeps track of the current line number (accessible with
 * {@link #getLineNumber getLineNumber} and {@link #setLineNumber setLineNumber}),
 * as well as the number of characters which have been consumed (accesible with
 * {@link #getCharNumber getCharNumber} and {@link #setCharNumber setCharNumber}).
 * 
 * <p>
 * The class usually keeps one character of look-ahead which has been read from
 * the underlying reader but not yet consumed by any scan method. If the
 * underlying reader is used later in some other capacity, this look-ahead
 * character may have to be taken into account. If a look-ahead character is
 * actually being stored, the {@link #lookAheadCharValid lookAheadCharValid}
 * method will return <code>true</code>, and the look-ahead character itself
 * can then be obtained using {@link #getLookAheadChar getLookAheadChar}. The
 * look-ahead character can be cleared using
 * {@link #clearLookAheadChar clearLookAheadChar}.
 */
public class ScanfReader extends Reader {
  ...
  private static ScanfFormat defaultBooleanFmt = new ScanfFormat("%b");
  ...
  /**
   * Reads characters into a portion of a character array. The method will block
   * until input is available, an I/O error occurs, or the end of the stream is
   * reached.
   * 
   * @param cbuf
   *          Buffer to write characters into
   * @param off
   *          Offset to start writing at
   * @param len
   *          Number of characters to read
   * @return The number of characters read, or -1 if the end of the stream is
   *         reached.
   * @throws IOException
   *           An I/O error occurred
   */
  ...
  private void matchKeyword(String s) throws IOException, ScanfMatchException {
    initChar();
    for (int i = 0; i < s.length(); i++) {
      char c = s.charAt(i);
      if (curChar == -1) {
      ...
      if (curChar != (int) c) {
        throw new ScanfMatchException("Expecting keyword \"" + s + "\"");
      }
      consumeAndReplaceChar();
    }
  }
   ...
   * 
   * <p>
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion character 'f'. The number itself may consist of (a) an
   * optional sign ('+' or '-'), (b) a sequence of decimal digits, with an
   * optional decimal point, (c) an optional exponent ('e' or 'E'), which must
   * by followed by an optionally signed sequence of decimal digits. White space
   * immediately before the number is skipped.
   * 
   * @param s
   *          Format string
   * @return Scanned double value
   * @throws ScanfMatchException
  ...
  public double scanDouble() throws IOException, ScanfMatchException {
    ...
    } catch (IllegalArgumentException e) { // can't happen
    }
    return val;
  }

  /**
   * Scan and return a double, using a pre-allocated <code>ScanfFormat</code>
   * object. This saves the overhead of parsing the format from a string.
   * 
   * @param fmt
   *          Format object
   * @return Scanned double value
  ...
  /**
   * Scan and return a float. The format string <code>s</code> takes the same
   * form as that described in the documentation for
   * <code>scanDouble(String)</code>.
   * 
   * @param s
   *          Format string
  ...
  /**
   * Scan and return a float, using a pre-allocated <code>ScanfFormat</code>
   * object. This saves the overhead of parsing the format from a string.
   * 
   * @param fmt
   *          Format object
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * either the conversion character 's', '[', 'q', 'S', or no conversion
   * character. If the conversion character is 's', then the returned string
   * corresponds to the next non-white-space sequence of characters found in the
   * input, with preceding white space skipped. If the conversion character is
   * '[', then the returned string corresponds to the next sequence of
   * characters which match those specified between the '[' and the closing ']'
   * (see the documentation for <code>ScanfFormat</code>). If the conversion
   * character is 'q', then the returned string corresponds to the next sequence
   * of characters delimited by double quotes, with preceding white space
   * skipped. If the conversion character is 'S', then the returned string
   * corresponds to the next sequence of characters delimited by either
   * white-space or double quotes, with preceding white space skipped. If there
   * is no conversion character, then the input must match the format string,
   * except that white space in the format may be matched by any amount of white
  ...
  /**
   * Scan and return a <code>String</code>, using the default format string
   * "%s".
   * 
   * @return Scanned <code>String</code>
   * @throws ScanfMatchException
  ...
  private void getQuotedString(int blimit) throws IOException, ScanfMatchException {
    consumeAndReplaceChar();
    ...
    if (curChar == '\n') {
      throw new ScanfMatchException("New line detected before end of quoted string");
    } else if (curChar == -1) {
      throw new ScanfMatchException("End of file detected before end of quoted string");
    } else // curChar == '"'
    {
      consumeAndReplaceChar();
    }
  ...
  private void getWhiteSpaceString(int blimit) throws IOException, ScanfMatchException {
    while (!Character.isWhitespace((char) curChar) && curChar != commentChar && curChar != -1 && bcnt < blimit) {
      buffer[bcnt++] = (char) curChar;
      consumeAndReplaceChar();
    }
  }
   ...
   * <code>ScanfFormat</code> object. This saves the overhead of parsing the
  ...
  public String scanString(ScanfFormat fmt) throws IOException, IllegalArgumentException {
    if (fmt.type == -1) { // then we just match the prefix and return null
      if (fmt.prefix != null) {
        matchString(fmt.prefix);
      }
      return null;
    ...
    if (fmt.type == 's') {
      ...
      if (curChar != '"') {
        throw new ScanfMatchException("Quoted string not found");
      }
    ...
    {
      while (fmt.matchChar((char) curChar) && curChar != commentChar && curChar != -1 && bcnt < blimit) {
        buffer[bcnt++] = (char) curChar;
        consumeAndReplaceChar();
      }
      if (bcnt == 0) {
        throw new ScanfMatchException("Input does not match '[" + fmt.cmatch + "]'");
      }
    }
    scanSuffix(fmt);
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion character 'b'. White space before the conversion is skipped.
   * The function returns true if the input matches the string
   * <code>"true"</code>, and false if the input matches the string
   * <code>"false"</code>. Other inputs will cause a ScanfMatchException to
   * be thrown. Any field width specification in the format is ignored.
   * 
   * @param s
   *          Format string
   ...
   * @see ScanfFormat
   */
  public boolean scanBoolean(String s) throws IOException, ScanfMatchException, IllegalArgumentException {
    return scanBoolean(new ScanfFormat(s));
  }

  /**
   * Scan and return a <code>boolean</code>, using the default format string
   * "%b".
   * 
   * @return Scanned <code>boolean</code>
   * @throws ScanfMatchException
   ...
   */
  public boolean scanBoolean() throws IOException, ScanfMatchException {
    boolean val = false;
    try {
   ...
   * @see ScanfReader#scanBoolean(String)
   */
  public boolean scanBoolean(ScanfFormat fmt) throws IOException, IllegalArgumentException {
    ...
    if (curChar == 't') {
      ...
      throw new ScanfMatchException("Expecting keywords \"true\" or \"false\"");
    }
    scanSuffix(fmt);
    return retval;
  }
  ...
  /**
   * Scan and return a character array, whose size is determined by the field
   * width specified in the format string (with a default width of 1 being
   * assumed if no width is specified).
   * 
   * <p>
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion characters 'c' or '['. If the conversion character is '[',
   * then each character scanned must match the sequence specified between the
   * '[' and the closing ']' (see the documentation for <code>ScanfFormat</code>).
   * 
   * <p>
  ...
  /**
   * Scan and return a character array, using the default format string "%c",
   * with the field width (number of characters to read) supplanted by the
   * argument <code>n</code>.
   * 
   * @param n
   *          Number of characters to read
   * @return Scanned character array
   * @throws IllegalArgumentException
   *           <code>n</code> not a positive number
   * @throws ScanfMatchException
   *           Input did not match format
   * @throws java.io.EOFException
   *           End of file
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion character 'c' or '['. If the conversion character is '[',
   * then each character scanned must match the sequence specified between the
   * '[' and the closing ']' (see the documentation for <code>ScanfFormat</code>).
   * 
   * <p>
  ...
  public char scanChar(ScanfFormat fmt) throws IOException, IllegalArgumentException {
    ...
    if (fmt.suffix == null && value == '\n') { // hack so that we won't block
                                                // if we've just scanned
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion character 'x'. The integer itself must be formed from the
   * characters [0-9a-fA-F], and white space which immediately precedes it is
   * skipped.
   * 
   * @param s
   *          Format string
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion character 'o'. The integer itself must be composed of the
   * digits [0-7], and white space which immediately precedes it is skipped.
   * 
   * @param s
   *          Format string
   * @return Scanned integer
  ...
  /**
   * Scan and return an octal (long) integer, using the default format string
   * "%o".
   * 
   * @return Scanned integer
   * @throws ScanfMatchException
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * the conversion character 'd'. The integer itself must consist of an
   * optional sign ('+' or '-') followed by a sequence of digits. White space
   ...
   * The format string <code>s</code> must have the form described by the
   * documentation for the class <code>ScanfFormat</code>, and must contain
   * one of the conversion characters "doxi".
   * 
   * <p>
   * Specifying the conversion characters 'd', 'o', or 'x' is equivalent to
   * calling (int versions of) <code>scanDec</code>, <code>scanOct</code>,
   * and <code>scanHex</code>, respectively.
   * 
   * <p>
   * If the conversion character is 'i', then after an optional sign ('+' or
   * '-'), if the number begins with an <code>0x</code>, then it is scanned
   * as a hex number; if it begins with an <code>0</code>, then it is scanned
   * as an octal number, and otherwise it is scanned as a decimal number. White
   * space preceding the number is skipped.
   * 
   * @param s
   *          Format string
   * @return Scanned integer
  ...
  /**
   * Scan and return a signed (long) integer. Functionality is identical to that
   * for <code>scanInt(String)</code>.
   * 
   * @param s
   *          Format string
   * @return Scanned integer
  ...
  /**
   * Scan and return a signed (long) integer, using the default format string
   * "%i".
   * 
   * @return Scanned integer
   * @throws ScanfMatchException
  ...
  public long scanLong(ScanfFormat fmt) throws IOException, IllegalArgumentException {
    ...
    } else // fmt.type == 'i';
    {
      ...
      } else { // scan unsigned decimal integer
        int i = 0;
        val = 0;
  ...
  /**
   * Gets the current character number (equal to the number of characters that
   * have been consumed by the stream).
   * 
  ...
  /**
   * Gets the current line number. The initial value (when the Reader is
   * created) is 1. A new line is recorded upon reading a carriage return, a
   * line feed, or a carriage return immediately followed by a line feed.
   * 
   ...
   * 
   * @return Look-ahead character, -1 if EOF has been reached, or 0 if no
   *         look-ahead character is being stored.
   */
  ...
  /**
   * Gets the comment character. A value of -1 means that no comment character
   * is set.
   * 
  ...
  /**
   * Sets the comment character. When encountered between scanned object, a
   * comment character causes all input from itself to the end of the current
   * line to be ignored as though it were whitespace. A comment character will
   * also terminate the scanning of a string. Otherwise, comment characters are
   * ignored within scanned objects, although if typical comment characters such
   * as <code>%</code> or <code>#</code> are used, this is unlikely ot be of
   * consequence. A value of -1 disables the comment character.
   * 
   * @param c
